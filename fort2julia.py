"""
    ...
    3. Fix more data type (parameter)
    4. Recongnize function such as log.
    5. Capitalization
    6. "fix_data" stuck in one line array data type, 
        e.g. data alpha / 0.781 , 0.209 , 0.0004, 0.0   , 0.0096,  0.0 /
...
patt_and = re.compile(r"\.and\.")
patt_or = re.compile(r"\.or\.")
...
patt_call = re.compile(r"(call)(\s+)([0-9a-zA-Z_]+)(\()([0-9a-zA-Z_+\-,\s]+)(\))")

# TODO []:
# Mess up with function such as log.
...
def fix_patt_and(line):

    line = patt_and.sub("&&", line)

    return line


def fix_patt_or(line):
    
    line = patt_or.sub("||", line)
...
def fix_patt_arr_parenthesis(line):
    ...
    # Fortran embedded function we don't want to edit (does not contain a "call").
    fort_embed_func = ["log", "abs", "sqrt", "exp", "float"]
    ...
    if patt_call.search(line) == None:
        ...
        for m in m_list:
            ...
            if not m[0] in fort_embed_func:

                m_to_list = []
                for substring in m:
                    m_to_list.append(substring)
                m_to_list[1] = "\("
                m_to_list[-1] = "\)"

                old_string = ""
                for substring in m_to_list:
                    old_string += substring
...
def fix_previous_line(lines, i_line):
    ...
    if patt_newline_math.search(lines[i_line]) != None:
        if patt_data_bgn.search(lines[i_line - 1]) == None:
            ...
            lines[i_line - 1] = lines[i_line - 1][:-1] + operator_str + "\n"
...
def fix_line(line):
    ...
    if patt_and.search(line) != None:

        line = fix_patt_and(line)
    
    if patt_or.search(line) != None:
    
        line = fix_patt_or(line)
